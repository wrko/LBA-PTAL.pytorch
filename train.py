...
for action in hp.actions:
    files.extend(glob.glob(os.path.join(path.air_extracted, f"*{action}*.npz")))
...
train_data_loader = data.DataLoader(train_dataset, batch_size=hp.batch_size, shuffle=True)
